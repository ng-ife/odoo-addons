...
class Repo(models.Model):
    ...
    _sql_constraints = [
        ('unique_path', 'unique(path)', 'The local path for a repository must be unique!'),
    ]
    ...
    def _check_path_validity(self):
        for rec in self:
            ...
            if os.path.isabs(rec.path):
                raise models.ValidationError('Repository path must be relative, not absolute!')
            if '.' in rec.path:
                raise models.ValidationError('Repository path must not contain dots!')
            if any(c.isspace() for c in rec.path):
                raise models.ValidationError('Repository path must not contain whitespaces!')
    ...
    def _compute_branch_count(self):
        for rec in self:
            rec.branch_count = self.env['ife.repo.branch'].search_count([('repo_id', '=', rec.id)])
