...
class EnvironmentModule(models.Model):
    ...
    repo_module_id = fields.Many2one(
        "ife.repo.module",
        string="Repo Module",
        required=True,
        domain="[('repo_id', '=', folder_repo_id)]",
    )
    ...
    _sql_constraints = [
        (
            "unique_name_folder_environment",
            "unique(name, folder_id, environment_id)",
            "The combination of module name and folder must be unique!",
        ),
    ]
        ...
        for rec in self:
            if rec.folder_id.repo_id != rec.repo_module_id.repo_id:
                raise ValidationError(
                    "The folder and module must belong to the same repository!"
                )
